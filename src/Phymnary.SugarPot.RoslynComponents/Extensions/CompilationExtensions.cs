...
public static class CompilationExtensions
{
    ...
    public static bool HasLanguageVersionGreaterThan(
        this Compilation compilation,
        LanguageVersion languageVersion
    )
    {
    ...
    public static bool HasAccessibleTypeWithMetadataName(
        this Compilation compilation,
        ...
        foreach (IModuleSymbol module in compilation.Assembly.Modules)
        {
            foreach (IAssemblySymbol referencedAssembly in module.ReferencedAssemblySymbols)
            {
                if (
                    ...
                    is not INamedTypeSymbol currentType
                )
                {
                    continue;
                }
    ...
    {
        ImmutableDictionary<T, INamedTypeSymbol>.Builder builder =
            ImmutableDictionary.CreateBuilder<T, INamedTypeSymbol>();
        ...
        {
            if (
                compilation.GetTypeByMetadataName(pair.Value)
                is not INamedTypeSymbol attributeSymbol
            )
            {
                typeSymbols = null;
